...
export default function AdminTrackBuses() {
  ...
  useEffect(() => {
    ...
    socket.on("adminReceiveLocation", handleLiveUpdate);

    return () => {
      socket.off("adminReceiveLocation", handleLiveUpdate);
    };
              ...
              <Marker key={busId} position={[loc.lat, loc.lng]} icon={busIcon}>
                <Popup>
                  <div className="adm-popup-content">
                    <strong className="adm-popup-bus">
                      Bus: {loc.busNo}
                    </strong>
                    <span className="adm-popup-time">
                      Active: {loc.updatedAt}
                    </span>
                  </div>
                </Popup>
              </Marker>
